...
DEFAULT_SUMMARY_CSV = Path(
    "tests/performance_test/gca_constLat/output/gca_constlat_SIMDPack_timing_summary.csv"
)
DEFAULT_REPEATS_CSV = Path(
    "tests/performance_test/gca_constLat/output/gca_constlat_SIMDPack_timing_repeats.csv"
)
DEFAULT_OLD_CSV = Path(
    "tests/performance_test/gca_constLat/output/gca_constlat_SIMDPack_timing.csv"
)
...
def method_label(method: str, width: int) -> str:
    if method == "pure_fp":
        if width == 1:
            return "FP64 AccuX-ConstLat Scalar"
        return f"FP64 AccuX-ConstLat SIMD Vector Width {width}"

    if method == "accux":
        if width == 1:
            return "AccuX ConstLat Scalar"
        return f"AccuX ConstLat SIMD Vector Width {width}"

    if method == "pure_fp_try":
        if width == 1:
            return "FP64 Try-API Scalar"
        return f"FP64 Try-API SIMD Vector Width {width}"

    if method == "accux_try":
        if width == 1:
            return "AccuX Try-API Scalar"
        return f"AccuX Try-API SIMD Vector Width {width}"
...
def normalize_timing_frame(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "time" in df.columns:
        ...
        # Repeats CSV has multiple rows per method/thread/width.
        # Plot median repeat timing by default.
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        required=True,
        help="Output figure path for the accux_constlat plot.",
    )
    parser.add_argument(
        "--title",
        default="GCA Constant-Latitude Full-Intersections Performance",
        help="Plot title for accux_constlat.",
    )
    parser.add_argument(
        "--try-title",
        default="GCA Constant-Latitude Try-API Performance",
        help="Plot title for try_gca_constlat_intersection.",
    )
    parser.add_argument(
        "--dpi",
        type=int,
    ...
    wrote_constlat = plot_method_block(
        df=df,
        output_path=output_path,
        title=args.title,
        ordered_series=constlat_series,
        dpi=args.dpi,
    )

    try_output_path = output_with_suffix(output_path, "_try")
    wrote_try = plot_method_block(
        df=df,
        output_path=try_output_path,
        title=args.try_title,
        ordered_series=try_series,
        dpi=args.dpi,
    )

    if not wrote_constlat and not wrote_try:
        raise ValueError(
            "No known rows found. Expected pure_fp/accux or pure_fp_try/accux_try."
        )

    if not wrote_constlat:
        print("No pure_fp/accux rows found; skipped full-intersections plot.")

    if not wrote_try:
        print("No pure_fp_try/accux_try rows found; skipped try-API plot.")
