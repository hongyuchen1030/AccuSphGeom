...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        required=True,
        help="Output figure path for the accux_gca plot.",
    )
    parser.add_argument(
        "--title",
        default="GCA-GCA Full-Intersections Performance",
        help="Plot title for accux_gca.",
    )
    parser.add_argument(
        "--try-title",
    ...
    wrote_gca = plot_method_block(
        df=df,
        output_path=output_path,
        title=args.title,
        ordered_series=gca_series,
        dpi=args.dpi,
    )
    ...
    if not wrote_gca and not wrote_try:
        raise ValueError(
